...
@pytest.fixture(scope='function')
def test_editor_user(app, db_session):
    """创建测试编辑者用户"""
    user = User(
...
@pytest.fixture(scope='function')
def test_viewer_user(app, db_session):
    """创建测试浏览者用户"""
    user = User(
...
@pytest.fixture(scope='function')
def editor_auth_headers(client, test_editor_user):
    """获取编辑者认证头"""
    response = client.post('/api/auth/login', json={
        'email': 'editor@example.com',
        'password': 'Test1234!',
...
@pytest.fixture(scope='function')
def viewer_auth_headers(client, test_viewer_user):
    """获取浏览者认证头"""
    response = client.post('/api/auth/login', json={
        'email': 'viewer@example.com',
        'password': 'Test1234!',
