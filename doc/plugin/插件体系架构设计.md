...
> 版本：1.0-draft ｜ 分支：feat-plugin ｜ 关联文档：[插件开发者指南](./plugin-developer-guide.md)
...
| type | 挂载位置 | 行为 |
|------|---------|------|
| `toolbar-button` | 表格视图工具栏 | **入口按钮**：点击后在右侧 Drawer 打开该插件的 side-panel |
| `side-panel` | 右侧 Drawer | **抽屉内容**：承载 iframe 沙箱渲染插件 UI |
| `base-menu` | Base 工具栏下拉菜单 | 菜单项点击后在**对话框**中打开插件沙箱（非 side-panel） |
| `record-detail-block` | 记录详情抽屉底部区块 | 承载 iframe 沙箱（携带 recordId） |
| `home-menu` | 首页操作区下拉菜单 | 菜单项点击后在对话框中打开插件沙箱（**全局作用域**，无需 Base 安装） |
| `dashboard-widget` | 仪表盘网格 | 作为自定义组件渲染 iframe 沙箱（组件类型 `plugin:<pluginId>`） |

> **`toolbar-button` 与 `side-panel` 是「入口 ↔ 内容」的配对关系**：宿主只为
> `side-panel` 提供抽屉容器，**不提供任何独立入口**——用户必须点击
> `toolbar-button` 才能打开对应插件的侧边面板。因此插件须**成对声明**二者；
> 仅声明 `side-panel` 而无 `toolbar-button` 的插件，其面板当前没有任何打开途径。
> 其余扩展点（base-menu / home-menu / record-detail-block / dashboard-widget）
> 各自有独立的宿主入口，不依赖 `toolbar-button`。
