...
export class DashboardService {
  ...
  async getDashboardsByBase(
    ...
    try {
      ...
      // 始终优先从后端 API 获取最新数据
      console.log(
        "[DashboardService] Fetching dashboards from API for base:",
        baseId,
      );
      ...
      console.warn(
        "[DashboardService] API failed, falling back to local cache",
      );
      const dashboards = await db.dashboards
        .where("baseId")
        .equals(baseId)
  ...
  async updateDashboard(
    ...
    try {
      ...
      if (changes.description !== undefined)
        apiData.description = changes.description;
  ...
  async duplicateDashboard(id: string, newName?: string): Promise<Dashboard> {
    try {
      console.log(`[dashboardService] 开始复制仪表盘 ${id}`);

      // 调用后端 API 复制仪表盘
      const apiDashboard = await dashboardApiService.duplicateDashboard(
        id,
        newName,
      );

      console.log(
        `[dashboardService] 复制成功，新仪表盘 ID: ${apiDashboard.id}`,
      );
      ...
      const localView: Dashboard = {
        id: apiDashboard.id,
        baseId:
          apiDashboard.base_id || (await this.getDashboard(id))?.baseId || "",
        name: apiDashboard.name,
        description: apiDashboard.description,
        widgets: (apiDashboard.widgets as unknown[]) || [],
        layout: apiDashboard.layout || {},
