...
import { dashboardService, type WidgetConfig } from '@/db/services/dashboardService'
import { dashboardShareService, type DashboardShare } from '@/db/services/dashboardShareService'
...
import type { RecordEntity, FieldEntity, TableEntity, Dashboard } from '@/db/schema'
...
// 组件类型定义
const widgetTypes = [
  { value: 'bar', label: '柱状图', icon: 'BarChart', description: '展示分类数据的对比' },
  { value: 'line', label: '折线图', icon: 'TrendCharts', description: '展示数据随时间的变化趋势' },
  { value: 'area', label: '面积图', icon: 'Histogram', description: '强调数量随时间变化的程度' },
  { value: 'pie', label: '饼图', icon: 'PieChart', description: '展示各部分占整体的比例' },
  { value: 'scatter', label: '散点图', icon: 'CircleCheck', description: '展示两个变量之间的关系' },
  { value: 'number', label: '数字卡片', icon: 'DataAnalysis', description: '突出显示关键指标' },
  { value: 'table', label: '数据表格', icon: 'Grid', description: '以表格形式展示详细数据' }
]

const aggregationTypes = [
  { value: 'count', label: '计数', description: '统计记录数量' },
  { value: 'countDistinct', label: '去重计数', description: '统计不重复值的数量' },
...
const canUseNumericAggregation = computed(() => {
  ...
  const field = fields.value.find(f => f.id === selectedWidget.value!.fieldId)
...
const filteredAggregationTypes = computed(() => {
  if (!selectedWidget.value?.fieldId) return aggregationTypes.filter(a => ['count', 'countDistinct'].includes(a.value))
  const field = fields.value.find(f => f.id === selectedWidget.value!.fieldId)
  if (!field) return aggregationTypes.filter(a => ['count', 'countDistinct'].includes(a.value))

  const numericTypes = [FieldType.NUMBER, FieldType.RATING, FieldType.PROGRESS, FieldType.AUTO_NUMBER]
  if (numericTypes.includes(field.type)) {
    return aggregationTypes
  }
  return aggregationTypes.filter(a => ['count', 'countDistinct'].includes(a.value))
})

// 加载仪表盘列表
async function loadDashboards() {
  if (!baseStore.currentBase) return
  dashboards.value = await dashboardService.getDashboardsByBase(baseStore.currentBase.id)
...
async function saveWidgets(showMessage = true) {
  ...
  try {
    ...
    await dashboardService.updateDashboardWidgets(currentDashboard.value.id, plainWidgets)
...
async function loadExistingShares() {
  ...
  existingShares.value = await dashboardShareService.getSharesByDashboard(currentDashboard.value.id)
}
...
async function copyAccessCode() {
  ...
  const success = await dashboardShareService.copyToClipboard(currentShare.value.accessCode)
  if (success) {
...
function renderWidget(widget: WidgetConfig) {
  ...
  // 获取该组件的数据
  const widgetRecords = records.value.filter(r => r.tableId === widget.tableId)
...
function getAggregationLabel(aggregation: string): string {
  return aggregationTypes.find(a => a.value === aggregation)?.label || aggregation
}
...
onUnmounted(() => {
  ...
  if (hasUnsavedChanges.value && currentDashboard.value) {
    const plainWidgets = JSON.parse(JSON.stringify(widgets.value))
    dashboardService.updateDashboardWidgets(currentDashboard.value.id, plainWidgets)
  }
})
</script>
    ...
    <div class="dashboard-toolbar">
      <div class="toolbar-left">
        <el-dropdown @command="showDashboardManager = true">
          <el-button text class="dashboard-selector">
            ...
            <span class="dashboard-name">{{ currentDashboard?.name || '选择仪表盘' }}</span>
      ...
      <div class="toolbar-right">
        ...
        <el-button
          v-if="currentDashboard"
          text
          @click="isEditingDashboard = true; isCreatingDashboard = false; dashboardForm = { name: currentDashboard.name, description: currentDashboard.description || '' }"
        >
    ...
    <div class="dashboard-content">
      ...
      <div class="widgets-grid">
        <div
          ...
          :key="widget.id"
          class="widget-card"
          :class="{ selected: selectedWidget?.id === widget.id, resizing: resizingWidget === widget.id }"
          :style="{
        ...
        <div v-if="widgets.length === 0" class="empty-dashboard">
          <el-empty description="暂无组件">
            ...
            <template #description>
              <p>点击上方"添加组件"创建图表</p>
              <p v-if="!currentDashboard" class="empty-hint">请先创建仪表盘</p>
            </template>
            <el-button v-if="!currentDashboard" type="primary" @click="isEditingDashboard = true">
              创建仪表盘
            </el-button>
          </el-empty>
        </div>
      ...
      <div v-if="selectedWidget" class="config-panel">
        <div class="panel-header">
          ...
          <div class="panel-actions">
            ...
            <el-tag v-else-if="isSaving" size="small" type="info">保存中...</el-tag>
          ...
          <el-form label-position="top" size="small">
            ...
            <div class="config-section">
              ...
              <el-form-item label="标题">
                <el-input v-model="selectedWidget.title" @change="debouncedSaveWidgets()" />
              </el-form-item>

              <el-form-item label="数据表">
                <el-select
                  :model-value="selectedWidget.tableId"
                  @update:model-value="val => { selectedWidget!.tableId = val; handleTableChange(val); debouncedSaveWidgets(); }"
                >
                  <el-option
            ...
            <div class="config-section">
              ...
              <el-form-item label="分组字段 (可选)">
                ...
                >
                  <el-option
                    v-for="field in fields.filter(f => f.tableId === selectedWidget.tableId)"
                    :key="field.id"
                    :label="field.name"
                    :value="field.id"
                  >
                    <el-icon class="field-icon">
                      <CollectionTag v-if="field.type === FieldType.SINGLE_SELECT || field.type === FieldType.MULTI_SELECT" />
              ...
              <el-form-item label="聚合方式">
                <el-select v-model="selectedWidget.aggregation" @change="debouncedSaveWidgets()">
                  <el-option
                  ...
                  >
                    <div class="aggregation-option">
                      <span>{{ agg.label }}</span>
                      <span class="aggregation-desc">{{ agg.description }}</span>
                    </div>
                  </el-option>
                </el-select>
                <div v-if="!canUseNumericAggregation && selectedWidget.aggregation !== 'count' && selectedWidget.aggregation !== 'countDistinct'" class="validation-warning">
            ...
            <div class="config-section">
              ...
              <el-form-item label="显示图例" v-if="selectedWidget.type !== 'number' && selectedWidget.type !== 'table'">
                <el-switch v-model="selectedWidget.config.showLegend" @change="debouncedSaveWidgets()" />
              </el-form-item>

              <el-form-item label="显示数值标签" v-if="selectedWidget.type === 'bar' || selectedWidget.type === 'pie'">
                <el-switch v-model="selectedWidget.config.showLabel" @change="debouncedSaveWidgets()" />
              </el-form-item>

              <el-form-item label="平滑曲线" v-if="selectedWidget.type === 'line' || selectedWidget.type === 'area'">
                <el-switch v-model="selectedWidget.config.smooth" @change="debouncedSaveWidgets()" />
              </el-form-item>
            </div>
            ...
            <div class="config-section save-section">
              ...
              >
                ...
                {{ isSaving ? '保存中...' : (hasUnsavedChanges ? '立即保存' : '已保存') }}
              </el-button>
            </div>
          </el-form>
        </el-scrollbar>
    ...
    >
      <div class="dashboard-manager">
        <div class="manager-header">
          <el-button type="primary" @click="isEditingDashboard = true; isCreatingDashboard = true; dashboardForm = { name: '', description: '' }">
            <el-icon><Plus /></el-icon>
            新建仪表盘
          </el-button>
        </div>
        ...
        <el-table :data="dashboards" style="width: 100%">
          <el-table-column prop="name" label="名称" min-width="150">
            <template #default="{ row }">
              <div class="dashboard-name-cell">
                <el-icon><DataAnalysis /></el-icon>
                <span>{{ row.name }}</span>
                <el-tag v-if="row.id === currentDashboard?.id" size="small" type="primary">当前</el-tag>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="200" fixed="right">
            <template #default="{ row }">
              <el-button link type="primary" @click="selectDashboard(row); showDashboardManager = false">
                打开
              </el-button>
              <el-button link @click="duplicateDashboard(row)">复制</el-button>
              <el-button link type="danger" @click="deleteDashboard(row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
      ...
      <el-form label-position="top">
        <el-form-item label="仪表盘名称" required>
          <el-input
            ...
            maxlength="50"
            show-word-limit
            @keyup.enter="isCreatingDashboard ? createDashboard() : updateDashboard()"
          />
        </el-form-item>
      ...
      <div class="share-dialog-content">
        ...
        <div v-if="currentShare && shareUrl" class="share-result-section">
          ...
          <div v-if="currentShare.accessCode" class="share-access-code">
            ...
            <el-button link type="primary" size="small" @click="copyAccessCode">
              <el-icon><CopyDocument /></el-icon>
              复制密码
            </el-button>
          </div>
          ...
          <div class="share-info">
            <el-tag v-if="currentShare.expiresAt" size="small" type="info">
              有效期至：{{ new Date(currentShare.expiresAt).toLocaleString() }}
            </el-tag>
            <el-tag v-else size="small" type="info">永久有效</el-tag>
            <el-tag v-if="currentShare.maxAccessCount" size="small" type="warning">
        ...
        <div v-if="existingShares.length > 0" class="share-list-section">
          ...
          <el-table :data="existingShares" size="small" style="width: 100%">
            ...
            <el-table-column label="有效期" width="120">
              <template #default="{ row }">
                <span v-if="row.expiresAt">{{ formatExpireTime(row.expiresAt) }}</span>
                <span v-else>永久</span>
              </template>
            </el-table-column>
            <el-table-column label="访问次数" width="100">
              <template #default="{ row }">
                ...
                <span v-if="row.maxAccessCount">/ {{ row.maxAccessCount }}</span>
              </template>
            </el-table-column>
            <el-table-column label="密码" width="80">
              <template #default="{ row }">
                <el-tag v-if="row.accessCode" size="small" type="warning">有</el-tag>
                <el-tag v-else size="small" type="info">无</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="操作" width="150">
              <template #default="{ row }">
                <el-button link type="primary" size="small" @click="copyShareUrlByToken(row.shareToken)">
                  复制
                </el-button>
                <el-button link type="danger" size="small" @click="deleteShare(row)">
                  删除
                </el-button>
              </template>
            </el-table-column>
