...
const localValue = ref<string | number | boolean | string[] | undefined>(undefined)

watch(() => props.modelValue, (newVal) => {
  if (typeof newVal === 'string' || typeof newVal === 'number' || typeof newVal === 'boolean' || Array.isArray(newVal)) {
...
const noValueOperators: FilterOperatorValue[] = [FilterOperator.IS_EMPTY, FilterOperator.IS_NOT_EMPTY]

const showInput = computed(() => {
  return !noValueOperators.includes(props.operator)
})

const multiSelectOperators: FilterOperatorValue[] = [FilterOperator.IS_ANY_OF, FilterOperator.IS_NONE_OF]

const multiSelectFieldTypes: FieldTypeValue[] = [FieldType.SINGLE_SELECT, FieldType.MULTI_SELECT, FieldType.MEMBER]

const showMultiSelect = computed(() => {
  return multiSelectOperators.includes(props.operator) &&
...
const selectOptions = computed(() => {
  const options = props.field.options?.options as Array<{ id: string; name: string; color?: string }> | undefined
  return options || []
})
...
<template>
  <div class="filter-value-input">
    <template v-if="showInput">
      ...
      <template v-else-if="field.type === FieldType.DATE || field.type === FieldType.CREATED_TIME || field.type === FieldType.UPDATED_TIME">
        <el-date-picker
          v-model="dateValue"
          type="date"
          placeholder="选择日期"
      ...
      <template v-else-if="field.type === FieldType.NUMBER || field.type === FieldType.RATING || field.type === FieldType.PROGRESS || field.type === FieldType.AUTO_NUMBER">
        <el-input-number
          v-model="numberValue"
          :controls="false"
          placeholder="输入数值"
