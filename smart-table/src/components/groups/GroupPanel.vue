...
const canAddMoreGroups = computed(() => {
  return (
    availableFields.value.length > localGroupBy.value.length &&
    !isMaxLevelReached.value
  );
});
...
function updateExpandState() {
  const checkExpanded = (nodes: GroupNode[]): boolean => {
    return nodes.every(
      (n) => n.isExpanded && (!n.children || checkExpanded(n.children)),
    );
  };
...
function updateGroups() {
  ...
  groupNodes.value = groupRecords(props.records as any[], config, props.fields);
}
