...
import { Upload, ArrowRight, ArrowLeft, Check, ArrowDown } from '@element-plus/icons-vue';
...
async function handleFileChange(file: File) {
  ...
  try {
    ...
    // 自动匹配字段
    fieldMappings.value = autoMatchFields(parsedData.value.columns, availableFields.value);
    ...
    for (let i = 0; i < totalRows; i += batchSize) {
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        try {
          ...
          if (!validation.valid) {
            result.failed++;
            result.errors.push(`第 ${rowIndex} 行: ${validation.errors.join(', ')}`);
            continue;
          }
          ...
          result.failed++;
          result.errors.push(`第 ${rowIndex} 行: ${error instanceof Error ? error.message : '导入失败'}`);
        }
      }
  ...
  >
    ...
    <div v-if="currentStep === 2" class="step-content">
      <div class="mapping-section">
        <h4>字段映射配置</h4>
        <p class="mapping-hint">将文件中的列映射到表格字段，未映射的列将被忽略</p>
      ...
      <div v-if="previewData.length > 0" class="preview-section">
        ...
        <div class="preview-table-wrapper">
          <el-table :data="previewData" border size="small" class="preview-table">
    ...
    <div v-if="currentStep === 3" class="step-content">
      <div v-if="importResult" class="result-section">
        <el-result
          :icon="importResult.failed === 0 ? 'success' : importResult.success === 0 ? 'error' : 'warning'"
          :title="importResult.failed === 0 ? '导入成功' : importResult.success === 0 ? '导入失败' : '部分导入成功'"
        >
          <template #sub-title>
          ...
          <template #extra>
            <div v-if="importResult.errors.length > 0" class="error-list">
              ...
              <el-button v-if="importResult.errors.length > 10" link type="primary" @click="downloadErrorLog">
                下载完整错误日志
              </el-button>
            </div>
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button v-if="currentStep < 2" type="primary" @click="nextStep" :disabled="!parsedData">
          下一步
