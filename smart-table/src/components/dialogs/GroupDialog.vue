...
import { ElDialog, ElButton, ElSelect, ElOption, ElTag, ElMessage } from 'element-plus'
...
const groupableFields = computed(() => {
  return props.fields.filter(f => groupableFieldTypes.includes(f.type as any))
})
...
function addGroupField() {
  ...
  const availableField = groupableFields.value.find(f => !groupBy.value.includes(f.id))
...
function getAvailableFields(currentIndex: number) {
  const usedFieldIds = groupBy.value
...
watch(() => props.initialGroupBy, (newGroupBy) => {
