...
const updateOption = <K extends keyof FieldOptions>(key: K, value: FieldOptions[K]) => {
  ensureOptions()
  localField.value = {
    ...localField.value,
...
const removeOption = (optionId: string) => {
  const options = localField.value.options?.options || []
  updateOption('options', options.filter(opt => opt.id !== optionId))
}

const updateOptionName = (optionId: string, name: string) => {
  const options = localField.value.options?.options || []
  updateOption('options', options.map(opt => 
...
const updateBooleanOption = (key: 'isRichText' | 'includeTime', val: string | number | boolean) => {
  updateOption(key, Boolean(val))
}

const showTextOptions = computed(() => localField.value.type === FieldType.TEXT)
const showNumberOptions = computed(() => localField.value.type === FieldType.NUMBER)
const showDateOptions = computed(() => localField.value.type === FieldType.DATE)
    ...
    <template v-if="showTextOptions">
      <div class="config-section">
        ...
        <el-switch
          ...
          @update:model-value="(val) => updateBooleanOption('isRichText', val)"
        />
      </div>
      <div class="config-section">
    ...
    <template v-if="showNumberOptions">
      <div class="config-section">
        ...
        <el-select
          ...
          @update:model-value="(val: 'number' | 'currency' | 'percent') => updateOption('format', val)"
          class="config-input"
        >
          <el-option
      ...
      <div v-if="localField.options?.format === 'currency'" class="config-section">
        <div class="config-label">货币符号</div>
        <el-select
          :model-value="localField.options?.currencySymbol || '¥'"
          @update:model-value="(val: string) => updateOption('currencySymbol', val)"
          class="config-input"
        >
          <el-option
    ...
    <template v-if="showDateOptions">
      <div class="config-section">
        ...
        <el-switch
          ...
          @update:model-value="(val) => updateBooleanOption('includeTime', val)"
        />
      </div>
    </template>
        ...
        <div class="options-list">
          ...
          >
            <el-color-picker
              ...
              @update:model-value="(val: string) => updateOptionColor(option.id, val)"
              size="small"
            />
            <el-input
              :model-value="option.name"
              @update:model-value="(val: string) => updateOptionName(option.id, val)"
              size="small"
              class="option-input"
            />
            <el-button
