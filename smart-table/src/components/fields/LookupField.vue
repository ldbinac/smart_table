...
const lookupValue = computed(() => {
  ...
  const values = recordIds
    .map(id => {
      const record = linkedRecords.value.find(r => r.id === id || r.id === (typeof id === 'object' ? (id as { id: string }).id : id))
...
watch(() => props.field.options, async (options) => {
  ...
  if (options?.linkedFieldId) {
    ...
    if (linkedTableId.value) {
      ...
      linkedField.value = fields.find(f => f.id === linkedFieldId.value) || null
    }
  }
}, { immediate: true })
...
<template>
  <div class="lookup-field">
    <span class="lookup-value">{{ formatValue(lookupValue as CellValue) }}</span>
  </div>
</template>
