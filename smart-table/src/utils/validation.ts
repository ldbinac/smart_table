...
export interface ValidationRule {
  ...
  message?: string
  pattern?: string
  validator?: (value: CellValue, record: RecordEntity, field: FieldEntity) => boolean
}
...
export class DataValidator {
  ...
  validateField(
    ...
    for (const rule of rules) {
      ...
      switch (rule.type) {
        ...
        case 'format':
          if (!this.isEmpty(value) && !this.validateFormat(value, field, rule.pattern)) {
            errors.push({
              fieldId: field.id,
              fieldName: field.name,
        ...
        case 'custom':
          if (rule.validator && record && !rule.validator(value, record, field)) {
            errors.push({
              fieldId: field.id,
              fieldName: field.name,
  ...
  private isUnique(fieldId: string, value: CellValue, excludeRecordId?: string): boolean {
  ...
  private validateFormat(value: CellValue, field: FieldEntity, pattern?: string): boolean {
    if (!pattern) {
