...
export interface ParseResult {
  type: 'literal' | 'field' | 'function' | 'operator' | 'number' | 'string' | 'boolean' | 'expression'
...
export class FormulaEngine {
  ...
  constructor(fields: FieldEntity[]) {
    ...
    this.fieldNameToId = new Map(fields.map(f => [f.name.toLowerCase(), f.id]))
  }
  ...
  private valueToExpression(value: CellValue, field: FieldEntity | undefined): string {
  ...
  validateFormula(formula: string): { valid: boolean; error?: string } {
    try {
      ...
      if (missingFields.length > 0) {
        return { valid: false, error: `未知字段引用: ${missingFields.join(', ')}` }
      }
      return { valid: true }
    } catch (error) {
      return { valid: false, error: error instanceof Error ? error.message : '公式验证失败' }
    }
  }
