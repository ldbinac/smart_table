...
export class ExcelExporter {
  ...
  private static formatValueForExcel(value: CellValue, field: FieldEntity): unknown {
    ...
    switch (field.type) {
      ...
      case FieldType.MEMBER:
        if (Array.isArray(value)) {
          return value.map(m => (typeof m === 'object' && m !== null && 'name' in m) ? m.name : String(m)).join(', ')
        }
        return value
      
      case FieldType.ATTACHMENT:
        if (Array.isArray(value)) {
          return value.map(f => (typeof f === 'object' && f !== null && 'name' in f) ? f.name : String(f)).join(', ')
        }
        return value
...
export class CSVExporter {
  ...
  private static formatValueForCSV(value: CellValue, field: FieldEntity): string {
    if (value === null || value === undefined) return ''
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const data = jsonData.map(item => headers.map(h => item[h] ?? ''))
