...
- **字段管理** - 支持 **27 种字段类型**，包含字段配置、排序、显示隐藏、默认值设置
...
- **插件体系** - 插件管理页完成上传安装、启停、升级回滚与卸载；支持 UI 插件（iframe 沙箱 + Vue 模板）与脚本插件（受限子进程），可读取表格勾选记录进行批处理
...
- **双形态插件**
  - **UI 插件** - 在 iframe 沙箱中运行（`sandbox="allow-scripts"`，opaque origin，无法访问宿主 Cookie/DOM），宿主注入 Vue 3 运行时，支持标准 Vue 模板语法（`v-model` / `v-for` / `@click` / 数据响应式），零构建单文件即可开发
  - **脚本插件** - 在受限子进程中运行，模块白名单 + 危险内建禁用，数据操作按**触发者身份**代理执行并双重鉴权
- **生命周期管理** - 上传安装、全局启停、Base 级安装/启停/移除、升级、回滚历史版本、卸载，全部由管理员在插件管理页完成（Base 页面零安装交互，打开即用）
- **两级配置** - `global` 与 `base` 作用域配置（base 深合并 global），写入时按插件声明的 `configSchema` 校验
- **双层权限** - 插件 manifest 权限声明 + 用户 RBAC，deny by default，宿主侧逐方法鉴权，越权调用返回 `PERMISSION_DENIED`
...
- **运行日志** - 记录运行状态、耗时、输出、结果与异常堆栈，支持按 Base 查询、结果弹窗与日志详情查看
